knnForPOG_MCF<-function(FM,knn_k = 10, num_dim = 25,ann_pseudotime){
  ...
  mcf_pca<-irlba_pca_res[grep(rownames(irlba_pca_res),pattern='\\POG_',invert=T),]
  pog_pca<-irlba_pca_res[grep(rownames(irlba_pca_res),pattern='\\POG_'),]
  
  all_id<-NULL
  all_pseudotime<-NULL

  for(ccle_exp in 1:length(rownames(pog_pca))){
    
    print(ccle_exp)
    
    ccle_id2<-rownames(pog_pca)[ccle_exp]
    all_id<-c(all_id,ccle_id2)
    
    ccle.mtx<-pog_pca[row.names(pog_pca)==ccle_id2,]
