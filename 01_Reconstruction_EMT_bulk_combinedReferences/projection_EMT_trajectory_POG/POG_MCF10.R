...
#setwd("~/Documents/EMT_paper_revision/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/01_Reconstruction_EMT_bulk/projection_EMT_trajectory_POG/MCF10/")
setwd("/home/annawiecek/EMT/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/01_Reconstruction_EMT_bulk/projection_EMT_trajectory_POG/MCF10")
...
setwd("~/Documents/EMT_paper_revision/TCGA_PurityScaledAnalysis/Data")
tcga <- read.table("POG570_TPM_expression.txt", header = TRUE, sep = "\t")
ensg_names <- unique(tcga$genes)
mart <- useMart(biomart = "ensembl", dataset = "hsapiens_gene_ensembl")
IDs <- getBM(attributes = c("ensembl_gene_id","hgnc_symbol"),filters = "ensembl_gene_id", values = ensg_names,mart = mart)
tcga<-merge(IDs,tcga,by.x="ensembl_gene_id",by.y="genes")
...
save(tcga, file = "pog_expr.RData")
tcga_samples <- colnames(tcga)[-1]


setwd("/home/annawiecek/EMT/TCGA_PurityScaledAnalysis/Data/")
load("pog_expr.RData")
gene_symbol <- tcga$gene_symbol
tcga$gene_symbol <- NULL
tcga <- log2(tcga +1)
tcga <- cbind(gene_symbol, tcga)
