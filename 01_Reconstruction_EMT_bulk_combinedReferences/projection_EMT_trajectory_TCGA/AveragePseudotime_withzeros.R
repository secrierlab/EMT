...
for (i in conditions) {
  ...
  pseudotime_add <- read.table(paste("KNN_projection_TCGA_to_",i,"_treated_cells_no_correction_primarytumor_withzeros.txt",sep = ''), header = TRUE,sep = "\t")
...
write.table(pseudotime, "AveragedPseudotime_lowconf_withzeros.txt",row.names=F,quote=F,sep="\t")
...
setwd("~/Documents/EMT_paper_revision/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/01_Reconstruction_EMT_bulk/projection_EMT_trajectory_TCGA/")
write.table(pseudotime, "AveragedPseudotime_highconf_withzeros.txt",row.names=F,quote=F,sep="\t")
