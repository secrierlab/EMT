####################################
#High confidence pseudotime analysis
...
input_cp_final <- read.table("AveragedPseudotime_highconf.txt", header = TRUE, sep = "\t")
...
write.table(input_cp_final2,file='HighConf_pseudotime_withEMT.txt',row.names=F,quote=F,sep='\t')
...
pdf(paste("TCGA_MET500_pseudotime_vs_emt_scores_mock_high_conf.pdf",sep="."),width=12,pointsize=12)
...
setwd("~/Documents/EMT_paper_revision/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/02_HMM_macrostases_EMT/output_dir_high_withzeros")
...
samples_with_HMM_and_MET500$HMM_states<-gsub(samples_with_HMM_and_MET500$HMM_states,pattern="3",replacement="epi")
samples_with_HMM_and_MET500$HMM_states<-gsub(samples_with_HMM_and_MET500$HMM_states,pattern="1",replacement="pEMT")
samples_with_HMM_and_MET500$HMM_states<-gsub(samples_with_HMM_and_MET500$HMM_states,pattern="2",replacement="mes")
...
pdf("Compare_EMT_scores_primary_HMMstates_with_MET500_highconf_full.pdf")
...
pdf("Pseudotime_values_for_HMMstate_highconf.pdf")
