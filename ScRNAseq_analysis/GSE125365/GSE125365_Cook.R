...
for (i in conditions) {
  ...
  setwd("~/Documents/EMT_paper_revision/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/ScRNAseqAnalysis/GSE125365/Cook/")
  #setwd("/home/annawiecek/EMT/TCGA_PurityScaledAnalysis/MCF10_Cook_averaged/ScRNAseqAnalysis/GSE125365/Cook/")
  ...
  for (a in samples) {
    print(a)
    expr.data <- read.table(file = paste(a,".rsem.genes.results.txt",sep = ""), header = TRUE, sep = "\t")
    tcga[[a]] <- expr.data[2]
  }
