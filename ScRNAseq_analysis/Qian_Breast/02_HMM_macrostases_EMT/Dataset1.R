...
load("TPM_data.RData") #This is a toy example of TCGA expression data
TCGA_GEXP_ALL<-TPM.data

setwd(input_dir)
input_file<-c("proj_pseudospace.txt")
...
for(state in 3){
  ...
  output_HMM<-data.frame(samples=rownames(TCGA_GEXP_ALL_sort),HMM_states=as.character(HMMpost[,1]),pseudospace=pseudospace_input_sort$mock)
