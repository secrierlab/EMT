...
setwd("~/Documents/EMT_paper_revision/Qian_Breast/01_Reconstruction_EMT_bulk /")
...
save(input_mock, file = "input_mock.RData")
save(input_tgfb, file = "input_tgfb.RData")

load("tcga.RData")
load("mock_norm2.RData")
load("tgfb_norm2.RData")
load("input_mock.RData")
load("input_tgfb.RData")
...
input_mock2 <- input_mock[apply(input_mock[,-1], 1, var) != 0, ]
combat_mock = ComBat(dat=as.matrix(input_mock2[,-1]), batch=batch)
combat_mock2<-data.frame(genes=input_mock2[,1],combat_mock)
...
input_tgfb2 <- input_tgfb[apply(input_tgfb[,-1], 1, var) != 0, ]

combat_tgfb = ComBat(dat=as.matrix(input_tgfb2[,-1]), batch=batch)
combat_tgfb2<-data.frame(genes=input_tgfb2[,1],combat_tgfb)
...
setwd("~/Documents/EMT_paper_revision/Qian_Breast/02_HMM_macrostases_EMT/")
epi_genes <- c("CDH1","DSP","OCLN")
mes_genes <- c("VIM","CDH2","FOXC2","SNAI1","SNAI2","TWIST1","FN1","ITGB6","MMP2","MMP3","MMP9","SOX10","GCS")
setwd("~/Documents/EMT_paper_revision/Qian_Breast/02_HMM_macrostases_EMT/data/")
...
#Check correlation between pseudotime and EMT score
library(ggplot2)
library(ggpubr)
pdf("EMT_vs_pseudotime.pdf", width = 5, height = 5)
